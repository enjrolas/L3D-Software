    ...
    try {
      ...
      if (lastHeard!=null)
      {
        ...
        for (Object variableName : activeVariables.keys())
        {
          ...
          if (activeVariables.getString((String)variableName).equals("string"))
          {
            String value=variable.getString("result");
            currentCore.setString((String)variableName, value);
          } else if (activeVariables.getString((String)variableName).equals("int32"))
          {
            int value=variable.getInt("result");
            currentCore.setInt((String)variableName, value);
          } else if (activeVariables.getString((String)variableName).equals("float"))
          {
            float value=variable.getFloat("result");
            currentCore.setFloat((String)variableName, value);
          }
        }
      }
    }
