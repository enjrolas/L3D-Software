...
void initDirectStreaming(String coreName)
{
  direct=true;
  println("getting IP address for core: "+coreName);
  address=getAddress(coreName);  
  println("IP address is "+address);
  udp = new UDP( this );
  // ... well, just verifies if it's really a multicast socket and blablabla
  println( "init as multicast socket ... "+udp.isMulticast() );
  println( "joins a multicast group  ... "+udp.isJoined() );
